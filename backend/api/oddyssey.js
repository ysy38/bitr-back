  ...
  try {
    ...
    const currentCycleQuery = `
      SELECT 
        cycle_id, 
        CASE 
          WHEN prize_pool::text ~ '^[0-9]+$' THEN prize_pool::numeric 
          ELSE 0 
        END as prize_pool_numeric
      FROM oracle.oddyssey_cycles 
      WHERE is_resolved = false 
      ORDER BY cycle_id DESC 
      LIMIT 1
    `;
    const currentCycleResult = await db.query(currentCycleQuery);
    const currentPrizePool = currentCycleResult.rows[0]?.prize_pool_numeric || 0;
    ...
    const transformedStats = {
      ...
      avgPrizePool: (parseFloat(stats.avg_prize_pool) || 0) / 1e18,  // Convert from WEI to STT
      currentPrizePool: (parseFloat(currentPrizePool) || 0) / 1e18,  // Convert from WEI to STT
