...
class EventDrivenBetSync {
  ...
  async setupEventListeners() {
    try {
      ...
      const liquidityAddedListener = this.contract.on('LiquidityAdded', async (
        ...
        try {
          console.log(`💰 ${this.serviceName}: LiquidityAdded event detected - Pool ID: ${poolId}, Provider: ${provider}, Amount: ${amount}`);
          ...
          // Handle liquidity addition as LP event (not a bet)
          await this.handleLiquidityAdded(poolId, provider, amount, event);
        } catch (error) {
          // ✅ CRITICAL FIX: Catch errors in event handler to prevent listener from stopping
          // If an error occurs, log it but don't let it crash the listener
    ...
    try {
      ...
      if (transactionHash && !transactionHash.startsWith('temp_')) {
        ...
        if (existingBet.rows.length > 0) {
          console.log(`⚠️ ${this.serviceName}: Bet already exists for transaction ${transactionHash} - skipping duplicate`);
      ...
      try {
        ...
        if (poolProgressResult.rows.length > 0) {
          ...
          wsService.updatePoolProgress(poolId, {
            ...
            totalBettorStake: progress.total_bettor_stake?.toString() || '0',
            ...
            betCount: betCount, // ✅ Include bet count
      ...
      somniaDataStreams.publishBet(poolId, bettor, amount, isForOutcome, event).catch(err => {
        console.warn(`⚠️ ${this.serviceName}: SDS bet publish failed (non-critical):`, err.message);
      });
      
      // Also publish pool progress update
      somniaDataStreams.publishPoolProgress(poolId).catch(err => {
        console.warn(`⚠️ ${this.serviceName}: SDS progress update failed (non-critical):`, err.message);
      });
  ...
  async handleLiquidityAdded(poolId, provider, amount, event, retryAttempt = 0) {
    try {
      ...
      // ✅ CRITICAL: Create bet record for LP addition so it shows in BetDisplay
      // LP additions are "NO bets" (is_for_outcome = false) - supporting creator side
      const transactionHash = event?.log?.transactionHash || event?.transactionHash;
      const blockNumber = event?.log?.blockNumber ? event.log.blockNumber.toString() : (event?.blockNumber ? event.blockNumber.toString() : '0');
      
      // Fetch pool info for bet record
      const db = require('../db/db');
      const poolInfoResult = await db.query(`
        SELECT 
          event_start_time, event_end_time, betting_end_time,
          league, category, home_team, away_team, title
        FROM oracle.pools 
        WHERE pool_id = $1
      `, [poolId]);
      ...
      // Create bet record for LP addition (is_for_outcome = false means NO bet / LP addition)
      await this.saveBetToDatabase({
        poolId: poolId.toString(),
        bettorAddress: provider,
        amount: amountBigInt.toString(), // Store as string (wei)
        isForOutcome: false, // ✅ LP addition is NOT for outcome (supporting creator)
        transactionHash: transactionHash,
        blockNumber: blockNumber,
        eventStartTime: poolInfo.event_start_time || 0,
        eventEndTime: poolInfo.event_end_time || 0,
        bettingEndTime: poolInfo.betting_end_time || 0,
        league: poolInfo.league || '',
        category: poolInfo.category || '',
        homeTeam: poolInfo.home_team || '',
        awayTeam: poolInfo.away_team || '',
        title: poolInfo.title || `Pool #${poolId}`
      });
  ...
  async updatePoolLPStake(poolId, amount) {
    try {
      ...
      // Update the pool's total_creator_side_stake
      ...
      const result = await db.query(`
        ...
        SET total_creator_side_stake = COALESCE(total_creator_side_stake, 0) + $1,
            updated_at = NOW()
        WHERE pool_id = $2
      `, [amountStr, poolId.toString()]);
      
      if (result.rowCount === 0) {
        throw new Error(`Pool ${poolId} not found in database`);
      }
      
      console.log(`✅ ${this.serviceName}: Pool ${poolId} LP stake updated by ${amountStr}`);
